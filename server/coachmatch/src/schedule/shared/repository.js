...
import { ScheduleInternalException } from './exceptions.js';
...
/**
 * Grava apenas `status` + `updatedAt`, exigindo que o schedule ainda exista e
 * continue em `expectedStatus` — o status lido pelo chamador antes de decidir
 * que a transição é permitida. Sem essa guarda, uma transição em voo
 * sobrescreveria um status gravado entre a leitura e a escrita.
 */
export const setScheduleStatus = async (scheduleId, { status, updatedAt, expectedStatus }) => {
  const docClient = createClient();
  try {
