...
/**
 * Limite superior da busca por conflito, com dois dias de folga.
 *
 * A sort key do GSI é comparada lexicograficamente, mas a sobreposição é
 * decidida em tempo absoluto: dois instantes escritos com offsets diferentes
 * podem se sobrepor e ainda assim ordenar ao contrário como texto. O desvio
 * entre as duas ordens é no máximo a amplitude de offsets válidos (±14h de cada
 * lado, 28h no total), então a folga garante que nenhum conflito real fique
 * fora do corte. Quem decide de fato é a checagem de sobreposição em memória.
 */
const conflictUpperBound = (endDateTime) =>
  new Date(new Date(endDateTime).getTime() + 2 * DAY_MS).toISOString();

/**
 * Schedules não cancelados do coach que podem colidir com a janela pedida.
 *
 * O limite superior na sort key não descarta nenhum conflito (ver
 * `conflictUpperBound`) — só evita varrer a agenda inteira do coach a cada criação.
 *
  ...
  try {
    return await queryAll(docClient, {
      ...
      ExpressionAttributeValues: {
        ':coachId': coachId,
        ':end': conflictUpperBound(endDateTime),
        ':cancelled': 'CANCELLED',
      },
    });
  } catch (err) {
