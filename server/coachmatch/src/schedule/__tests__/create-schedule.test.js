...
describe('create-schedule › index', () => {
  ...
  it.each([['abc'], [null], [-50], [0], [{}]])('preserva price recebido (%s), como a Lambda Python', async (price) => {
    sendMock
      .mockResolvedValueOnce({ Items: [] })
    ...
    const item = await createSchedule(COACH_ID, { ...VALID_BODY, price });
...
describe('create-schedule › repository', () => {
  ...
  it('limita a query de conflito à janela pedida, com folga para offsets, e pagina', async () => {
    sendMock
    ...
    // Dois dias além do fim pedido: a sort key ordena por texto, a sobreposição
    // é decidida em tempo absoluto, e offsets diferentes desalinham as duas.
    const bound = firstPage.input.ExpressionAttributeValues[':end'];
    expect(new Date(bound).getTime() - new Date(VALID_BODY.endDateTime).getTime())
